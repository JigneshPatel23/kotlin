...
class PackageParts(val packageFqName: String) {
    ...
    fun addTo(builder: JvmPackageTable.PackageTable.Builder) {
        if (parts.isNotEmpty()) {
            val jvmPackageNames = mutableListOf<String>()
            ...
            builder.addPackageParts(JvmPackageTable.PackageParts.newBuilder().apply {
                ...
                writePartsOutsidePackage(partsOutsidePackage, jvmPackageNames)
            })

            builder.addAllJvmPackageName(jvmPackageNames)
        }
    ...
    private fun JvmPackageTable.PackageParts.Builder.writePartsOutsidePackage(parts: List<String>, jvmPackageNames: MutableList<String>) {
        ...
        for ((packageInternalName, partsInPackage) in parts.groupBy { it.packageName }.toSortedMap()) {
            ...
            if (packageFqName !in jvmPackageNames) {
                jvmPackageNames.add(packageFqName)
            }
            val packageId = jvmPackageNames.indexOf(packageFqName)
